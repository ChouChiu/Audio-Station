...
HomePage::HomePage(QWidget* parent) : QWidget(parent) {
    ...
    connect(m_songBtn, &qfw::PushButton::clicked, this, &HomePage::selectSong);
    ...
    connect(m_accBtn, &qfw::PushButton::clicked, this, &HomePage::selectAccompaniment);
    ...
    connect(m_outputBtn, &qfw::PushButton::clicked, this, &HomePage::selectOutput);
    ...
    connect(m_cancelBtn, &qfw::PushButton::clicked, this, &HomePage::cancelRequested);
    btnRow->addWidget(m_cancelBtn);
    m_startBtn = new qfw::PrimaryPushButton;
    connect(m_startBtn, &qfw::PushButton::clicked, this, &HomePage::startRequested);
...
QString HomePage::songPath() const { return m_songPath; }
QString HomePage::accompanimentPath() const { return m_accompanimentPath; }
QString HomePage::outputPath() const { return m_outputPath; }
void HomePage::setSongPath(const QString& path) {
    m_songPath = path;
    m_songPathEdit->setText(path);
}
void HomePage::setAccompanimentPath(const QString& path) {
    m_accompanimentPath = path;
    m_accPathEdit->setText(path);
}
void HomePage::setOutputPath(const QString& path) {
    m_outputPath = path;
    m_outputPathEdit->setText(path);
}
bool HomePage::autoFindEnabled() const { return m_autoFindSwitch->isChecked(); }
bool HomePage::autoAlignEnabled() const { return m_alignSwitch->isChecked(); }
int HomePage::strength() const { return m_strengthSlider->value(); }
int HomePage::sigmaTime() const { return m_sigmaCombo->currentData().toInt(); }
QString HomePage::algorithmKey() const { return m_algorithmCombo->currentData().toString(); }

void HomePage::setStatus(const QString& text) {
    m_statusLabel->setText(text);
}

void HomePage::setProgress(int value) {
    m_progressBar->setValue(value);
}

void HomePage::setProcessing(bool running) {
...
void HomePage::selectAccompaniment() {
    const QString filePath = QFileDialog::getOpenFileName(
