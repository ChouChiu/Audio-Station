...
def test_lossless_stereo_mimo_cancels_crossfeed():
    sample_rate = 16_000
    ...
    right_vocal = np.sin(2 * np.pi * 523 * time) * 0.32
    mix = np.stack(
        [
    ...
    output = process_audio(
        mix, np.stack([left_ref, right_ref]), sample_rate, 1.0, Algorithm.LOSSLESS, 8
    )
...
def test_bypass_and_cancellation(scene):
    ...
    assert np.array_equal(
        process_audio(stereo, np.stack([instrumental] * 2), sample_rate, 0, Algorithm.LOSSLESS, 8),
    ...
    with pytest.raises(ProcessingCancelled):
        process_audio(
            stereo, np.stack([instrumental] * 2), sample_rate, 1, Algorithm.LOSSLESS, 8, token
        )
