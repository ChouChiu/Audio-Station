...
$\mathbf{v}(t)$。默认 `spectral_mask` 算法在 STFT 域估计随时间和频率变化的复数 $2\times2$ 传递矩阵：
...
$M_\alpha=1-\alpha(1-M)$ 连续控制处理量，输出始终是原混音复频谱乘以实数掩码；默认路径不会把参考波形或预测参考复频谱直接相减。

`direct` 标识为兼容已有命令而保留。它仍以直接相减测量候选残差，但最终只把残差幅度投影到原混音频谱，不会输出参考波形或反极性参考。
...
flowchart LR
    ...
    enabled -->|否| choose{"提取算法"}
    warp --> choose
    choose -->|默认| mask["复数传递估计<br/>置信度 Wiener 掩码"]
    choose -->|A/B| direct["2×2 直接残差<br/>原混音安全投影"]
    mask --> center{"开启中置处理？"}
    direct --> center
...
算法先拟合一次，根据预测残差降低人声、欢呼和异常瞬态的权重，再执行一次加权拟合。`sigma` 的 1、3、8 或 16 秒决定统计上下文；默认 8 秒偏向稳定。
...
`direct` 对每个增益窗口以加正则的最小二乘估计实数传递矩阵：
...
为减少人声、欢呼和瞬态峰值干扰，算法先拟合一次，依据残差能量降低异常帧权重，再执行一次加权拟合。直接相减只生成内部候选残差：

$$
\mathbf R=\mathbf Y-\widehat{\mathbf H}\mathbf X
$$

候选残差不能直接写出，因为参考中若含有现场不存在的原版歌词，$\mathbf R$ 会包含可听见的反极性原词。实现改为比较候选残差与原混音的联动功率，得到限制在 $[0.05,1]$ 的实数掩码：

$$
M=\operatorname{clip}\!\left(\sqrt{\frac{P_R}{P_Y+\varepsilon}},\ 0.05,\ 1\right),
\qquad
\widehat{\mathbf V}_c=\mathbf Y_c M
$$

最终相位和全部复频谱都来自现场原混音。候选残差幅度大于原混音时只会直通，不会把多出的反相参考带入输出。`direct` 名称仅为命令行兼容保留，用户界面显示为“安全直接残差”。
...
这两个选项都会改变空间感，不属于默认频域核心。是否启用应通过同一素材的试听对比决定。

## 输出保护与验证

处理结束后会把非有限值替换为有限数，并在需要时避免峰值越界。参考对消输出 24-bit PCM WAV。

算法测试覆盖时间偏移、局部漂移、反极性、频率相关房间传递、无关参考、参考独有改词否决、矩阵串扰、块接缝、中置聚焦和弱人声保护。合成信号指标只用于发现实现回归；真实素材仍需以相同输入和参数比较 `spectral_mask` 与 `direct`，重点听人声大小、齿音、呼吸、和声、混响尾音与观众声是否自然。
